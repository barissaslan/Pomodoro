    ...
    public class iTalk_NumericUpDown : Control
    {
        ...
        protected override void OnKeyPress(KeyPressEventArgs e)
        {
            ...
            try
            {
                if (KeyboardNum == true)
                {
                    _Value = int.Parse((_Value).ToString() + e.KeyChar.ToString().ToString());
                }
                if (_Value > _Maximum)
                {
                    _Value = _Maximum;
    ...
    public class iTalk_TabControl : TabControl
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            for (int TabIndex = 0; TabIndex <= TabCount - 1; TabIndex++)
            {
                ...
                {
                    Rectangle TabRect = new Rectangle(new Point(GetTabRect(TabIndex).Location.X - 35, GetTabRect(TabIndex).Location.Y - 2), new Size(GetTabRect(TabIndex).Width + 36, GetTabRect(TabIndex).Height - 8));
